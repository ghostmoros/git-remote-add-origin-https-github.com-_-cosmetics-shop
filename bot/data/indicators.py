...
def adx(high: pd.Series, low: pd.Series, close: pd.Series, period: int = 14):
    ...
    ADX measures trend *strength* regardless of direction (0..100). A common
    rule: > 25 means a strong trend, < 20 means range/chop. Very useful on gold
    to avoid trading the choppy ranges and only act when a trend has legs.
    """
    up_move = high.diff()
    down_move = -low.diff()
